 ...
 */
public class LobbyRMI extends UnicastRemoteObject implements Remote {
    ...
    /**
     * Map associating gameId of started games to their GameControllerRMI (given to the payers for that game)
     */
    private final Map<Integer, GameControllerRMI> controllers;

    /**
     * Unique instance of this class (implementing Singleton pattern)
     */
    private static LobbyRMI instance = null;
    ...
    private LobbyRMI() throws RemoteException {
        super();
     ...
     * @return Unique instance of LobbyRMI
     */
    public synchronized static LobbyRMI getInstance() throws RemoteException {
        if(instance==null)
            instance = new LobbyRMI();
        return instance;
    }
    ...
    public synchronized void joinRoom(int gameId, GameListenerClientRMI playerListener) throws LobbyException, RemoteException {
        GameListenerServerRMI lis = new GameListenerServerRMI(playerListener);
        GameController controller = lobby.joinRoom(gameId, lis);
        mapLis.put(playerListener.getPlayer(), lis);
        if(controller != null) {     // the game started, i must notify all players in the room
            GameControllerRMI controllerRMI = new GameControllerRMI(controller, mapLis.values());
            controllers.put(gameId, controllerRMI);
            mapLis.values().forEach(l -> l.updateGameController(controllerRMI));
        }
    }
     ...
     */
    public synchronized void reconnectPlayer(GameListenerClientRMI playerListener) throws LobbyException, RemoteException, ConnectionException, GameStatusException, InvalidPlayerException {
        GameListenerServerRMI lis = new GameListenerServerRMI(playerListener);
        GameControllerRMI controller = controllers.get(lobby.reconnectPlayer(lis).getGameId());
        mapLis.put(playerListener.getPlayer(), lis);
        controller.reconnectPlayer(lis);
        lis.updateGameController(controller);
    }
